...
<%
	...
	else
	{
		...
		else
		{
			if( null != strOnline && strOnline.length() > 0 )
			{
				rstx = statement.executeQuery("select ID, realuname, accstat, buyid, pregold, postgold, online, chardbupdate, chardblastonline from users WHERE chardbupdate>=1");
			}
			else
			{
				rstx = statement.executeQuery("select ID, realuname, accstat, buyid, pregold, postgold, online, chardbupdate, chardblastonline from users WHERE system!=1");
			}
			while (rstx.next())
			{
			...
			try
			{
				...
				else
				{
					...
					if (onlineBool == true)
					{
						...
						if ((role.status.level > top3onlevel) && (roleid >= 160000))
						{
...
%>
<center>
<img src="http://perfectworld.sytes.net:82/index/gamemap/?postype=3">
